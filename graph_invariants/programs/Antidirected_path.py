...
if __name__ == "__main__":
    ...
    length = 2*k-1
    ad = get_antidirected_path(length)
    try:
        p = find_subgraph_isomorphism(D, bitmask_to_sage_graph(ad, length)).__next__()
        # draw(D, p)
        logger.info("Found antidirected path of length 2k-1")
